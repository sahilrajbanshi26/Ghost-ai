...
export function EditorShell({ ownedProjects, sharedProjects, children, workspaceTitle, projectId, isOwner = false }: EditorShellProps) {
  ...
  return (
    <LiveblocksProvider authEndpoint="/api/liveblocks-auth">
      <RoomProvider key={projectId ?? "empty-project"} id={projectId ?? "ghost-ai-shell"} initialPresence={{ cursor: null, thinking: false, tabId: "" }} initialStorage={{} as never}>
        <div className="min-h-screen bg-background">
          ...
          <main className="h-screen overflow-hidden pt-14">
            <div className="relative h-[calc(100vh-3.5rem)] min-h-0">
              ...
              {isCopilotOpen && projectId && <AISidebar projectId={projectId} roomId={projectId} isOpen={isCopilotOpen} onClose={() => setIsCopilotOpen(false)} />}
            </div>
          </main>
          <ProjectDialogs
